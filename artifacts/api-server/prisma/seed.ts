...
async function main() {
  ...
  const plantRows = await Promise.all([
    ...
    prisma.plant.create({ data: { name: "Sapele Steam",         type: "GAS_STEAM", installedMw: 1020, availableMw: 100,  actualMw: 90,   state: "Delta",       latitude: 5.89,   longitude: 5.692,  status: "CONSTRAINED", gencoOrgId: transcorpOrg!.id,  paf: 10,  notes: "Aged steam units, limited gas supply." } }),
    prisma.plant.create({ data: { name: "Sapele NIPP",          type: "GAS_OCGT",  installedMw: 450,  availableMw: 220,  actualMw: 180,  state: "Delta",       latitude: 5.892,  longitude: 5.694,  status: "CONSTRAINED", gencoOrgId: ndphcOrg!.id,      paf: 49,  notes: "Gas constraint." } }),
    ...
    prisma.plant.create({ data: { name: "Olorunsogo NIPP",      type: "GAS_OCGT",  installedMw: 750,  availableMw: 220,  actualMw: 180,  state: "Ogun",        latitude: 6.967,  longitude: 3.215,  status: "CONSTRAINED", gencoOrgId: ndphcOrg!.id,      paf: 29,  notes: "Gas pipeline constraints." } }),
    ...
    prisma.plant.create({ data: { name: "Calabar NIPP",         type: "GAS_OCGT",  installedMw: 561,  availableMw: 380,  actualMw: 320,  state: "Cross River", latitude: 5.15,   longitude: 8.288,  status: "OPERATING",   gencoOrgId: ndphcOrg!.id,      paf: 68,  alsoKnownAs: "Odukpani NIPP", notes: "NERC Q1 2025 canonical name. Merged from Odukpani NIPP per merge_audit 2026-05-09." } }),
    ...
    prisma.plant.create({ data: { name: "Rivers IPP",           type: "GAS_OCGT",  installedMw: 180,  availableMw: 0,    actualMw: 0,    state: "Rivers",      latitude: 4.789,  longitude: 7.116,  status: "OUT",         gencoOrgId: transcorpOrg!.id,  paf: 0,   notes: "Zero output NERC Sep 2025." } }),
    prisma.plant.create({ data: { name: "Omoku Power",          type: "GAS_OCGT",  installedMw: 265,  availableMw: 0,    actualMw: 0,    state: "Rivers",      latitude: 5.35,   longitude: 6.65,   status: "OUT",         gencoOrgId: ndphcOrg!.id,      paf: 0,   notes: "Zero output NERC Sep 2025." } }),
    prisma.plant.create({ data: { name: "Ibom Power",           type: "GAS_OCGT",  installedMw: 191,  availableMw: 0,    actualMw: 0,    state: "Akwa Ibom",   latitude: 4.5728, longitude: 7.5552, status: "OUT",                                        paf: 0,   notes: "Zero output NERC Sep 2025. Ibom Power Co. Ltd, owned by Akwa Ibom State Govt." } }),
  ...
  await prisma.dataQualityFlag.createMany({
    data: [
      ...
      // ── merge_audit 2026-05-09 — 4 pre-resolved actions ──────────────────
      {
        recordId: "PLT-calabar-odukpani",
        reason: "NAME_INCONSISTENCY",
        severity: "HIGH",
        detail: "Odukpani NIPP and Calabar NIPP are the same 561MW NDPHC plant in Cross River State (Odukpani LGA). Canonical name per NERC Q1 2025 is 'Calabar NIPP'. Former name preserved in alsoKnownAs field.",
        status: "RESOLVED" as const,
        notes: "Merged: Calabar NIPP kept as canonical. alsoKnownAs = 'Odukpani NIPP'. Source: merge_audit_1778315942102.json.",
        resolvedAt: new Date("2026-05-09"),
      },
      {
        recordId: "SUB-kaduna-kumbotso2",
        reason: "DUPLICATE_RECORD",
        severity: "HIGH",
        detail: "Record 'Kaduna (Kumbotso 2)' flagged as misnamed duplicate of Kaduna Mando 330kV. Kumbotso is a Kano district — the correct Kumbotso entry (Kano, 330kV, 600MVA) is already canonical in the substations table.",
        status: "RESOLVED" as const,
        notes: "Record was never in the dataset. Kumbotso is correctly attributed to Kano. Kaduna Mando is the canonical Kaduna 330kV substation. Source: merge_audit_1778315942102.json.",
        resolvedAt: new Date("2026-05-09"),
      },
      {
        recordId: "SUB-damaturu-maiduguri-loop",
        reason: "WRONG_TABLE",
        severity: "MEDIUM",
        detail: "Entry 'Damaturu - Maiduguri Loop' was a transmission line mistakenly placed in the substations table. Damaturu and Maiduguri are correctly represented as separate 330kV substations.",
        status: "RESOLVED" as const,
        notes: "Record was never present in substations. Damaturu (330kV, 240MVA, Yobe) and Maiduguri (330kV, 360MVA, Borno) are distinct and correctly seeded. Source: merge_audit_1778315942102.json.",
        resolvedAt: new Date("2026-05-09"),
      },
      {
        recordId: "ORG-ondoerc",
        reason: "DUPLICATE_RECORD",
        severity: "MEDIUM",
        detail: "ONDOERC is an alternate name for OSERC (Ondo State Electricity Regulatory Commission). Canonical name per state law is OSERC.",
        status: "RESOLVED" as const,
        notes: "ONDOERC was never added to the organisations table. OSERC is not currently in scope for the federal war room registry. No action required. Source: merge_audit_1778315942102.json.",
        resolvedAt: new Date("2026-05-09"),
      },
    ],
  });
